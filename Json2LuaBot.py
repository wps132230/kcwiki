...
for mapid in maplist:
    ...
    if not response:
        ...
        for point in j:
            ...
            fleetNum = 1
            for fleet in point['fleets']:
                jsonStr += '\t'*(ind+3) + '["' + str(fleetNum) + '"] = {' + '\n'
                fleetNum += 1
                jsonStr += '\t'*(ind+4) + u'["制空"] = ' + '0' + ',' + '\n'
                jsonStr += '\t'*(ind+4) + u'["阵型"] = "' + formation[fleet['formation']-1] + '",' + '\n'
                jsonStr += '\t'*(ind+4) + u'["详细"] = ' + str(fleet['ships']).replace('[','{').replace(']','}') + ',' + '\n'
                jsonStr += '\t'*(ind+3) + '},' + '\n'
